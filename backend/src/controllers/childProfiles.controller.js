...
const createChildProfile = asyncHandler(async (req, res) => {
  const profile = await childProfilesService.createChildProfile(req.user.id, req.body, req.token);
  return success(req, res, { profile }, 201);
});

const getChildProfiles = asyncHandler(async (req, res) => {
  const profiles = await childProfilesService.getChildProfiles(req.user.id, req.token);
  return success(req, res, { profiles });
});

const updateChildProfile = asyncHandler(async (req, res) => {
  const profile = await childProfilesService.updateChildProfile(req.params.id, req.user.id, req.body, req.token);
